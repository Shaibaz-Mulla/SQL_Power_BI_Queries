...
FROM
	...
	(SELECT 
		...
		--BNK_REC_Date
		--CASE WHEN OuterQuery.BNK_REC_Date IS NOT NULL THEN CONCAT_WS('/',DAY(OuterQuery.BNK_REC_Date),MONTH(OuterQuery.BNK_REC_Date),YEAR(OuterQuery.BNK_REC_Date)) ELSE 'N.A.' END AS 'BNK_REC_Date',
		--HO_REC_Date
		--CASE WHEN OuterQuery.HO_REC_Date IS NOT NULL THEN CONCAT_WS('/',DAY(OuterQuery.HO_REC_Date),MONTH(OuterQuery.HO_REC_Date),YEAR(OuterQuery.HO_REC_Date)) ELSE 'N.A.' END AS 'HO_REC_Date',
        --CMO_REC_Date
		--CASE WHEN OuterQuery.CMO_REC_Date IS NOT NULL THEN CONCAT_WS('/',DAY(OuterQuery.CMO_REC_Date),MONTH(OuterQuery.CMO_REC_Date),YEAR(OuterQuery.CMO_REC_Date)) ELSE 'N.A.' END AS 'CMO_REC_Date',
        --CMO_TO_HO_REC_Date
        --CASE WHEN OuterQuery.CMO_TO_HO_REC_Date IS NOT NULL THEN CONCAT_WS('/',DAY(OuterQuery.CMO_TO_HO_REC_Date),MONTH(OuterQuery.CMO_TO_HO_REC_Date),YEAR(OuterQuery.CMO_TO_HO_REC_Date)) ELSE 'N.A.' END AS 'CMO_TO_HO_REC_Date',
        --HO_TO_BNK_REC_Date
		--CASE WHEN OuterQuery.HO_TO_BNK_REC_Date IS NOT NULL THEN CONCAT_WS('/',DAY(OuterQuery.HO_TO_BNK_REC_Date),MONTH(OuterQuery.HO_TO_BNK_REC_Date),YEAR(OuterQuery.HO_TO_BNK_REC_Date)) ELSE 'N.A.' END AS 'HO_TO_BNK_REC_Date',
        --FINAL_DEST_REC_Date
		...
		--CAT_TAT
		(CASE 
			WHEN OuterQuery.CAT_REC_Date IS NOT NULL AND OuterQuery.FINAL_DEST_REC_Date IS NOT NULL 
			THEN DATEDIFF(D,OuterQuery.CAT_REC_Date,OuterQuery.FINAL_DEST_REC_Date)
			ELSE 0
		END) AS 'CAT_TAT',
		...
		*/
		...
		--TOTAL_TAT
        (CASE 
			WHEN OuterQuery.FT_Creation_Date IS NOT NULL AND OuterQuery.FINAL_DEST_REC_Date IS NOT NULL 
			THEN DATEDIFF(D,OuterQuery.FT_Creation_Date,OuterQuery.FINAL_DEST_REC_Date)
			ELSE 0
		END) AS 'TOTAL_TAT'
	...
	FROM
		...
		(SELECT 
			...
			(CASE 
				WHEN BaseQuery.CatID = 17  
				THEN (SELECT vText FROM Item_Data WHERE FieldID = 68 AND ItemID = BaseQuery.ItemID)
				WHEN BaseQuery.CatID = 18  
				THEN (SELECT vText FROM Item_Data WHERE FieldID = 64 AND ItemID = BaseQuery.ItemID)
			END) AS 'Barcode',
			--Finding Document #
			(CASE 
				WHEN BaseQuery.CatID = 17  
				THEN (SELECT vText FROM Item_Data WHERE FieldID = 69 AND ItemID = BaseQuery.ItemID) 
				WHEN BaseQuery.CatID = 18  
				THEN (SELECT vText FROM Item_Data WHERE FieldID = 67 AND ItemID = BaseQuery.ItemID) 
			END) AS 'Document_#',
			--Finding Document Creation Date 
			(SELECT i.CreatedDate FROM Item i WHERE i.ItemID = BaseQuery.ItemID) AS 'FT_Creation_Date',
			--Finding credit/debit note created by
			...
			--Finding Document Date 
			(CASE 
				WHEN BaseQuery.CatID = 17  
				THEN (SELECT vDate FROM Item_Data WHERE FieldID = 71 AND ItemID = BaseQuery.ItemID)
				WHEN BaseQuery.CatID = 18  
				THEN (SELECT vDate FROM Item_Data WHERE FieldID = 66 AND ItemID = BaseQuery.ItemID)
			END) AS 'Document_Date',
			--Finding document amount
			(CASE 
				WHEN BaseQuery.CatID = 17  
				THEN (SELECT vNumber FROM Item_Data WHERE FieldID = 70 AND ItemID = BaseQuery.ItemID)
				WHEN BaseQuery.CatID = 18  
				THEN 0
		...
		FROM (SELECT DISTINCT i.ItemID, i.CatID FROM Item i WHERE i.CatID in (17,18)) AS BaseQuery) AS OuterQuery) AS EdgeQuery;
